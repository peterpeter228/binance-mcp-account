...
export function createAdvancedTools(binanceClient: BinanceClient): Tool[] {
  return [
    {
...
export async function handleAdvancedTool(name: string, args: any, binanceClient: BinanceClient): Promise<any> {
  ...
  try {
    switch (name) {
      ...
      case 'binance_analyze_portfolio_risk':
        ...
        let riskLevel = 'LOW';
        if (marginUsageRatio > 80) riskLevel = 'HIGH';
        else if (marginUsageRatio > 50) riskLevel = 'MEDIUM';
        ...
        return {
          ...
          data: {
            ...
            spotAssets: {
              ...
              totalValue: spotBalances.reduce((sum, bal) => sum + parseFloat((bal as any).total || '0'), 0),
            },
            recommendations: [
