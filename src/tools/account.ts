...
export function createAccountTools(binanceClient: BinanceClient): Tool[] {
      ...
      inputSchema: {
        ...
        properties: {
          symbol: {
            type: 'string',
            description:
              '指定查询的交易对，如"BTCUSDT"、"ETHUSDT"。不填则返回所有有持仓的合约。支持USDT永续、币本位合约等。',
...
export async function handleAccountTool(name: string, args: any, binanceClient: BinanceClient): Promise<any> {
  const spotAPI = new SpotAPI(binanceClient);
  const futuresAPI = new FuturesAPI(binanceClient);
